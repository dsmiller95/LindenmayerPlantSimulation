...
namespace ProceduralToolkit.SplineMesh {
    ...
    public class SplineSmoother : MonoBehaviour {
        ...
        private void SmoothNode(SplineNode node) {
            ...
            // In ProceduralToolkit.SplineMesh, the node direction is not relative to the node position. 
            var controlPoint = dir + pos;

            // We only set one direction at each spline node because ProceduralToolkit.SplineMesh only support mirrored direction between curves.
